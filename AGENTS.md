...
agent 在了解用户论文方向之前，必须先主动询问模版要求。这是整个流程的第 0 步，不能跳过。

- 用户可能在对话中随口提到格式要求，也可能在 `template/` 目录下放了 docx 样例，或者在 `input/constraints/` 下有文字描述。
- agent 必须显式询问："你有没有模版或格式要求？比如老师发的 docx 样例、或者文字描述的字体字号要求？"
- 模版信息确认后才能进入选题阶段。
- 如果用户说"没有"，也视为已确认（模版信息为空），可以继续。
- 模版可以是以下任意形式：
  - 一个 `.docx` 文件（放在 `template/` 目录下），agent 需要拆包读取其中的样式定义
  - 一段文字描述（如"标题：黑体二号加粗居中，正文：宋体小四 1.5 倍行距"），agent 记录到 `workspace/当前项目/00_模版要求.md`
- 确认模版后，在后续的写作和导出阶段必须遵守模版约束。
- **关键**：agent 拆包读取模板 docx 后，必须将提取到的格式规则回填到 `input/constraints/格式与写作要求.md` 的对应字段（字体、字号、行距、页码等），不能只记在对话上下文或 `workspace/` 中。导出管线和校验环节需要依赖该文件。
...
- 导出 docx 必须使用项目管线：`python -m src.app export --project-root <项目目录>` 或 `python -m src.app refresh --project-root <项目目录>`。
- **严禁** agent 自行编写一次性脚本来构建 docx（如手写 XML、自定义 zip 打包等）。
- 如果管线的输出不满足模板要求（缺少封面、目录、致谢等模块），应通过增强 `src/export/docx.py` 来支持，而不是绕过管线。
- DocxExporter 已内置引用标记上标（`<w:vertAlign w:val="superscript"/>`），agent 不需要自行处理引用格式。
...
| 阶段 | 可以做的事 | 禁止做的事 |
|------|-----------|-----------|
| 0. 模版未确认 | 询问模版要求、检查 `template/` 和 `input/constraints/` | 讨论选题方向 |
| 1. 模版已确认，题目未定 | 讨论选题方向、给候选题目 | 给正式检索式、推进到资料收集 |
...
在第 2 步（生成检索式、提示用户准备资料）中，agent 必须使用以下准确路径和格式说明，不能只说"放到 input/ 目录"：

**必须逐条告知用户的内容：**

1. **参考文献**：放下 `input/references/raw/`。格式为 GB/T 7714（如知网导出格式），每条包含作者、标题、刊名、年份、卷期、页码。
2. **查新引文**：放下 `input/references/raw/`（可与参考文献合并为一个文件）。格式为引文格式，每条必须包含摘要。
3. **文献 PDF 原文（可选）**：放下 `input/references/raw/pdf/`。
4. **明确说明三种材料的区别**：参考文献是标引依据，查新引文用于内容理解，PDF 用于深度阅读。

**建议提示模板：**

```text
你下一步需要做的是准备文献资料。
1. 下载相关论文 PDF，放到 input/references/raw/pdf/（可选）
2. 把查新引文和参考文献文本放到 input/references/raw/（必需）
   - 参考文献格式：GB/T 7714，如 "[1] 作者. 题名[J/OL]. 刊名, 年份, 卷(期): 页码."
   - 查新引文格式：同上但必须附带论文摘要
完成后回复我：资料已放入。
```
...
- `docs/Prompt策略.md`
- `input/topic/任务需求.md`
- `workspace/当前项目/00_模版要求.md`（模版信息确认后优先读取）
...
3. **主动询问模版要求**：检查 `template/` 下是否有 docx 文件、`input/constraints/` 下是否有格式描述。没有则直接确认，进入选题。
4. 运行 `python -m src.app guide --project-root <项目目录>` 获取下一步指令。
5. 如可自动推进，再运行 `python -m src.app advance --project-root <项目目录>`。
6. 完成论文后，运行 `python -m src.app archive --project-root <项目目录>` 归档当前任务。
7. 只有在需要精细控制某个阶段时，再单独调用 `ingest`、`plan`、`write-prepare`、`refresh`、`export`。
